...
- [x] **Uploads use a PRIVATE bucket + short-lived signed URLs.** The app no longer
      emits public object URLs: `uploadToStorage` stores the object path, `meta.files`
      keeps `{name, path}`, and reads go through `GET /api/files`, which requires the
      workspace **edit key** and verifies the path is inside the workspace's own
      namespace before minting a 10-min signed URL (302 redirect). **Create the
      `cofounder-uploads` bucket as PRIVATE** (public toggle OFF) in Supabase — the code
      assumes it; a public bucket would still be world-readable by direct URL. Legacy
      `{url}` file entries keep rendering for back-compat.
...
- [x] **Artwork is 100% original.** Every raster mirrored from the live site has been
      **removed** and replaced by original SVGs generated by `scripts/gen-art.mjs`
      (`npm run art:gen`) — hero, chapter covers, department + brand-vibe cards, product
      icons, and decorative bits. `public/` now contains zero `.jpg/.png/.gif`; nothing
      copyrighted ships. Re-run the generator to tweak the house palette.
